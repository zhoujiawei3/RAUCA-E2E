...
class Detections:
    ...
    def display_ZJW(self, pprint=False, show=False, save=False, crop=False, render=False, save_dir=Path(''),name='result'):
        for i, (im, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            if im is not None:
                ...
                print("im is None")
            str = f'{name} {i + 1}/{len(self.pred)}: {im.shape[0]}x{im.shape[1]} '
            if pred is not None:
                for c in pred[:, -1].unique():
                    n = (pred[:, -1] == c).sum()  # detections per class
                    str += f"{n} {self.names[int(c)]}{'s' * (n > 1)}, "  # add to string
            ...
            if save:
                f = self.files[i]
                print(f)
                im.save(os.path.join(save_dir,name))  # save
                print(f"{'Saved' * (i == 0)} {f}", end=',' if i < self.n - 1 else f' to {save_dir}\n')
            if render:
                self.imgs[i] = np.asarray(im)
    def print(self):
    ...
    def save_ZJW(self, save_dir='runs/hub/exp',name='result'):
        save_dir = increment_path(save_dir, exist_ok=save_dir != 'runs/hub/exp', mkdir=True)  # increment save_dir
        self.display_ZJW(save=True, save_dir=save_dir,name=name)  # save results
