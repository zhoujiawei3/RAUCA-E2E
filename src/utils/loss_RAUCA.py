...
from utils.general import bbox_iou_RAUCA_loss,xyxy2xywhn
from utils.torch_utils import is_parallel
...
class ComputeLoss:
    # Compute losses
    def __init__(self, model, autobalance=False,model_type='car'):
        super(ComputeLoss, self).__init__()
        ...
        if model_type=='attack_plane':
            self.type_number=4
        elif model_type=='boat_complicated':
            self.type_number=8
        else:
            self.type_number=2
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        loss=torch.max(p_clone_clone[:,:,5+self.type_number])
